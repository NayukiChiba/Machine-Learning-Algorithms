...
def demo_scalers():
    ...
    ax_orig.scatter(
        X[0, 0], X[0, 1], c="red", s=150, marker="*", label="异常值", edgecolors="black"
    )
...
def demo_encoders():
    ...
    encoded = [0, 1, 2]
    bars = ax1.barh(
        categories, encoded, color=["#FF6B6B", "#4ECDC4", "#45B7D1"], edgecolor="black"
    )
    ax1.set_xlabel("编码值")
    ...
    for i in range(3):
        for j in range(3):
            ax2.text(
                ...
                ha="center",
                va="center",
                color="white" if onehot_data[i, j] == 1 else "black",
                fontsize=14,
                fontweight="bold",
            )
