...
def demo_clone():
    ...
    print(
        f"参数相同: {rf.get_params()['n_estimators'] == rf_clone.get_params()['n_estimators']}"
    )
...
def demo_class_weight():
    ...
    print(
        f"balanced - 少数类F1: {classification_report(y_test, clf_balanced.predict(X_test), output_dict=True)['1']['f1-score']:.3f}"
    )
