...
def demo_linear_regression():
    ...
    ax2.bar(x_pos + width, lasso.coef_, width, label='Lasso', color='#45B7D1', alpha=0.7)
    ax2.set_xlabel('特征')
    ax2.set_ylabel('系数值')
...
def demo_ensemble_models():
    ...
    colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4']
    bars = ax.barh(list(scores.keys()), list(scores.values()), color=colors, edgecolor='black')
    ax.set_xlabel('准确率')
    ax.set_title('集成学习模型对比 (鸢尾花数据集)')
    ax.set_xlim(0, 1.1)
    for bar, score in zip(bars, scores.values()):
        ax.annotate(f'{score:.3f}', xy=(score + 0.02, bar.get_y() + bar.get_height()/2),
...
def demo_svm():
    ...
    for ax, kernel, title in zip(axes, kernels, titles):
        ...
        ax.scatter(X_moons[y_moons==0, 0], X_moons[y_moons==0, 1], c='#FF6B6B', label='类别 0', edgecolors='white', s=50)
        ax.scatter(X_moons[y_moons==1, 0], X_moons[y_moons==1, 1], c='#4ECDC4', label='类别 1', edgecolors='white', s=50)
...
def demo_clustering():
    ...
    colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4']
    for i in range(4):
        ax1.scatter(X[y_true==i, 0], X[y_true==i, 1], c=colors[i], s=40, alpha=0.7, edgecolors='white')
    ax1.set_title('真实标签')
    ax1.set_xlabel('特征 1')
    ax1.set_ylabel('特征 2')
    ...
    ax2 = axes[1]
    for i in range(4):
        ax2.scatter(X[labels_km==i, 0], X[labels_km==i, 1], c=colors[i], s=40, alpha=0.7, edgecolors='white')
    ...
    for label in unique_labels:
        if label == -1:
            ax3.scatter(X[labels_db==-1, 0], X[labels_db==-1, 1], c='gray', marker='x', s=40, label='噪声', alpha=0.5)
        else:
            ax3.scatter(X[labels_db==label, 0], X[labels_db==label, 1], c=colors[label%4], s=40, alpha=0.7, edgecolors='white')
...
def demo_dimensionality_reduction():
    ...
    for i, name in enumerate(iris.target_names):
        ax1.scatter(X_pca[y==i, 0], X_pca[y==i, 1], c=colors[i], label=name, s=50, alpha=0.7, edgecolors='white')
    ...
    ax2 = axes[1]
    for i, name in enumerate(iris.target_names):
        ax2.scatter(X_tsne[y==i, 0], X_tsne[y==i, 1], c=colors[i], label=name, s=50, alpha=0.7, edgecolors='white')
    ...
    plt.tight_layout()
    plt.savefig('outputs/sklearn/07_dimensionality_reduction.png', dpi=150, bbox_inches='tight')
    plt.close()
