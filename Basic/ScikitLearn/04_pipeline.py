...
def demo_set_params():
    ...
    print(f"修改前: PCA n_components={pipe.named_steps['pca'].n_components}, SVM C={pipe.named_steps['svm'].C}")
    
    # 格式: 步骤名__参数名
    pipe.set_params(pca__n_components=3, svm__C=10)
    
    print(f"修改后: PCA n_components={pipe.named_steps['pca'].n_components}, SVM C={pipe.named_steps['svm'].C}")
