...
def demo_load_datasets():
    ...
    bars = ax2.bar(
        iris.target_names, counts, color=colors, edgecolor="black", alpha=0.8
    )
    ax2.set_xlabel("类别")
    ax2.set_ylabel("样本数量")
...
def demo_generate_data():
    ...
    # regression
    X_reg_2d, y_reg_2d = make_regression(
...
def demo_first_model():
    ...
    ax2.plot(
        k_values, test_scores, "s-", color="#FF6B6B", label="测试集", lw=2, markersize=6
    )
