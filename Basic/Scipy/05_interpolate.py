...
def demo_interp2d():
    ...
    # 原始网格数据
    ax1 = axes[0]
    im1 = ax1.imshow(Z, extent=[0, 4, 0, 4], origin='lower', cmap='viridis', aspect='auto')
    ax1.scatter(X.flatten(), Y.flatten(), c='white', s=30, marker='s', edgecolors='black')
    ...
    im2 = ax2.imshow(Z_fine, extent=[0, 4, 0, 4], origin='lower', cmap='viridis', aspect='auto')
    ax2.scatter(X.flatten(), Y.flatten(), c='white', s=30, marker='s', edgecolors='black', label='原始点')
    ...
    ax3.scatter(X.flatten(), Y.flatten(), Z.flatten(), c='red', s=50, label='原始数据点')
    ax3.set_title('插值曲面')
    ax3.set_xlabel('x')
    ax3.set_ylabel('y')
    ax3.set_zlabel('z')
...
def demo_rbf():
    ...
    im = ax2.imshow(Z_grid, extent=[0, 4, 0, 4], origin='lower', cmap='viridis', aspect='auto')
    ax2.scatter(x, y, c='white', s=50, edgecolors='black', label='原始点')
    ax2.scatter(test_points[:, 0], test_points[:, 1], c='red', s=100, marker='*', label='测试点')
