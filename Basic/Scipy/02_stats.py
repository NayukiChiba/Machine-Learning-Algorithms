...
def demo_distributions():
    ...
    ax3.axvline(binom.mean(), color='r', linestyle='--', lw=2, label=f'期望值 = {binom.mean()}')
    ...
    ax4.axvline(poisson.mean(), color='r', linestyle='--', lw=2, label=f'期望值 λ = {poisson.mean()}')
...
def demo_descriptive_stats():
    ...
    # 直方图 + 正态拟合
    ax1 = axes[0]
    n, bins, patches = ax1.hist(data, bins=15, density=True, alpha=0.7, 
                                 color='steelblue', edgecolor='black', label='数据分布')
    
    # 拟合正态分布
    mu, std = stats.norm.fit(data)
    x = np.linspace(data.min(), data.max(), 100)
    ax1.plot(x, stats.norm.pdf(x, mu, std), 'r-', lw=2, label=f'正态拟合 N({mu:.1f}, {std:.1f}^2)')
    
    # 标记均值和中位数
    ax1.axvline(np.mean(data), color='green', linestyle='--', lw=2, label=f'均值 = {np.mean(data):.1f}')
    ax1.axvline(np.median(data), color='orange', linestyle=':', lw=2, label=f'中位数 = {np.median(data):.1f}')
    ...
    ax2.text(1.3, np.mean(data), stats_text, fontsize=10, verticalalignment='center',
...
def demo_percentiles():
    ...
    for p in key_p:
        ...
        ax.annotate(f'P{p}={val:.1f}', (val, p), textcoords="offset points", 
