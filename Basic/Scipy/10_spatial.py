...
def demo_distance():
    ...
    for i in range(4):
        for j in range(4):
            ax1.text(j, i, f'{dist_matrix[i, j]:.2f}', ha='center', va='center', fontsize=10)
    ...
    for i, p in enumerate(points):
        ax2.annotate(f'P{i}({p[0]},{p[1]})', p, textcoords="offset points", 
...
def demo_kdtree():
    ...
    ax1.scatter(query_point[0], query_point[1], c='red', s=200, marker='*', 
                zorder=5, label='查询点')
    ax1.scatter(points[idx, 0], points[idx, 1], c='green', s=100, 
                zorder=5, label='最近邻')
    
    # 画到最近邻的线
    ax1.plot([query_point[0], points[idx, 0]], [query_point[1], points[idx, 1]], 
            'g--', lw=2)
    ...
    for i, (d, j) in enumerate(zip(dists, idxs)):
        ...
        ax2.plot([query_point[0], points[j, 0]], [query_point[1], points[j, 1]], 
                color=colors[i], linestyle='--', lw=1.5)
        ax2.annotate(f'#{i+1} (d={d:.2f})', points[j], textcoords="offset points",
                    xytext=(5, 5), fontsize=9)
    
    # 画搜索半径
    circle = plt.Circle(query_point, dists[-1], fill=False, color='orange', 
                        linestyle='--', linewidth=2, label=f'搜索半径 r={dists[-1]:.2f}')
...
def demo_convex_hull():
    ...
    ax.scatter(points[hull.vertices, 0], points[hull.vertices, 1], 
               c='red', s=100, zorder=5, label='凸包顶点')
...
def demo_voronoi():
    ...
    # 使用 voronoi_plot_2d
    spatial.voronoi_plot_2d(vor, ax=ax, show_vertices=True, line_colors='blue', 
...
def demo_delaunay():
    ...
    # Delaunay
    ax2.triplot(points[:, 0], points[:, 1], tri.simplices, 'b-', lw=1.5, alpha=0.5, label='Delaunay')
    
    # Voronoi
    spatial.voronoi_plot_2d(vor, ax=ax2, show_vertices=False, line_colors='red',
                            line_width=1.5, line_alpha=0.7, point_size=0)
