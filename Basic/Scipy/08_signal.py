...
def demo_filter():
    ...
    ax3.plot(freqs[:len(freqs)//2], fft_noisy[:len(freqs)//2], 'gray', alpha=0.5, label='带噪信号')
    ax3.plot(freqs[:len(freqs)//2], fft_filtered[:len(freqs)//2], 'r-', lw=2, label='滤波后')
...
def demo_fft():
    ...
    ax1.set_xlim(0, 0.4)
    ax1.set_title(f'时域信号: $sin(2\\pi \\cdot {freq1}t) + 0.5 \\cdot sin(2\\pi \\cdot {freq2}t)$')
    ax1.set_xlabel('时间 (s)')
    ax1.set_ylabel('幅值')
    ax1.grid(True, alpha=0.3)
    ...
    ax2.scatter(peak_freqs, magnitude[peaks], c='red', s=100, zorder=5, label='频率峰值')
    for freq in peak_freqs:
        ax2.annotate(f'{freq:.0f} Hz', (freq, magnitude[np.abs(xf[:len(t)//2] - freq).argmin()]),
    ...
    ax3.plot(t, 0.5*np.sin(2*np.pi*freq2*t), 'r-', lw=2, label=f'{freq2} Hz 成分')
    ax3.set_xlim(0, 0.4)
...
def demo_peak_finding():
    ...
    ax1.scatter(x[peaks], y[peaks], c='red', s=100, zorder=5, label=f'峰值 ({len(peaks)}个)')
    ax1.axhline(0.5, color='gray', linestyle='--', alpha=0.5, label='高度阈值 = 0.5')
    
    for i, peak in enumerate(peaks):
        ax1.annotate(f'({x[peak]:.1f}, {y[peak]:.2f})', (x[peak], y[peak]),
    ...
    ax2.bar(x_pos - bar_width/2, properties['peak_heights'], bar_width, 
            label='峰值高度', color='steelblue', alpha=0.7)
    ax2.bar(x_pos + bar_width/2, prominences, bar_width, 
            label='突出度', color='coral', alpha=0.7)
