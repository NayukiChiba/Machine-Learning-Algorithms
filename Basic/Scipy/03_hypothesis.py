...
def demo_ttest():
    ...
    ax1.fill_between(
        x[x >= t_crit],
        t_dist.pdf(x[x >= t_crit]),
        color="red",
        alpha=0.4,
        label=f"拒绝域 (α={alpha})",
    )
    ax1.fill_between(
        x[x <= -t_crit], t_dist.pdf(x[x <= -t_crit]), color="red", alpha=0.4
    )

    # 标记 t 统计量
    ax1.axvline(
        t_stat, color="green", linestyle="--", lw=2, label=f"t统计量 = {t_stat:.2f}"
    )
    ...
    positions = [1, 2]
    bp = ax2.boxplot(
        [group1, group2], positions=positions, widths=0.6, patch_artist=True
    )
    ...
    ax2.scatter(
        np.ones(len(group1)) + np.random.normal(0, 0.05, len(group1)),
        group1,
        alpha=0.5,
        s=30,
        color="blue",
    ...
    ax2.scatter(
        2 * np.ones(len(group2)) + np.random.normal(0, 0.05, len(group2)),
        group2,
        alpha=0.5,
        s=30,
        color="red",
...
def demo_chi2():
    ...
    ax1.bar(
        x - width / 2, observed, width, label="观察频数", color="steelblue", alpha=0.7
    )
...
def demo_anova():
    ...
    ax2.plot(
        x, stats.f.pdf(x, df1, df2), "b-", lw=2, label=f"F分布 (df1={df1}, df2={df2})"
    )
    ...
    ax2.axvline(
        f_stat, color="green", linestyle="--", lw=2, label=f"F统计量 = {f_stat:.2f}"
    )
