...
LGB_OUTPUTS = os.path.join(OUTPUTS_ROOT, "LightGBM")
...
def visualize_results(
    ...
    """
    os.makedirs(LGB_OUTPUTS, exist_ok=True)
    ...
    plt.tight_layout()
    filepath = os.path.join(LGB_OUTPUTS, "04_confusion_matrix.png")
    ...
    plt.tight_layout()
    filepath = os.path.join(LGB_OUTPUTS, "05_decision_boundary.png")
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    print(f"保存图像: {filepath}")
    ...
    plt.bar(["x1", "x2"], importances, color=["steelblue", "coral"])
    plt.title("特征重要性", fontsize=14, fontweight="bold")
    plt.xlabel("特征")
    plt.ylabel("重要性")
    plt.grid(True, axis="y", alpha=0.3)
    plt.tight_layout()
    filepath = os.path.join(LGB_OUTPUTS, "06_feature_importance.png")
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    print(f"保存图像: {filepath}")
