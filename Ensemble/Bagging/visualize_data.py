...
BAGGING_OUTPUTS = os.path.join(OUTPUTS_ROOT, "Bagging")
...
def visualize_data(data: DataFrame):
    ...
    args:
        data(DataFrame): 输入数据
    """
    os.makedirs(BAGGING_OUTPUTS, exist_ok=True)
    ...
    plt.tight_layout()
    filepath = os.path.join(BAGGING_OUTPUTS, "01_class_distribution.png")
    ...
    plt.tight_layout()
    filepath = os.path.join(BAGGING_OUTPUTS, "02_data_scatter.png")
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    print(f"保存图像: {filepath}")
    ...
    plt.tight_layout()
    filepath = os.path.join(BAGGING_OUTPUTS, "03_correlation_heatmap.png")
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    print(f"保存图像: {filepath}")
