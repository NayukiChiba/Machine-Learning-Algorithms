...
EM_OUTPUTS = os.path.join(OUTPUTS_ROOT, "EM")
...
def visualize_data(data: DataFrame):
    ...
    args:
        data(DataFrame): 输入数据
    """
    os.makedirs(EM_OUTPUTS, exist_ok=True)
    ...
    plt.tight_layout()
    filepath = os.path.join(EM_OUTPUTS, "01_raw_scatter.png")
    ...
    plt.tight_layout()
    filepath = os.path.join(EM_OUTPUTS, "02_correlation_heatmap.png")
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    print(f"保存图像: {filepath}")
