...
def visualize_data(data: DataFrame):
    ...
    plt.tight_layout()
    filepath = os.path.join(PCA_OUTPUTS, "01_class_distribution.png")
    ...
    plt.title("原始特征分布（前两维）", fontsize=14, fontweight="bold")
    plt.xlabel(data.columns[0])
    plt.ylabel(data.columns[1])
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    filepath = os.path.join(PCA_OUTPUTS, "02_raw_scatter.png")
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    print(f"保存图像: {filepath}")

    # 3. 相关性热力图
    plt.figure(figsize=(6, 5))
    corr = data.drop(columns=["label"]).corr(numeric_only=True)
    sns.heatmap(corr, annot=True, fmt=".2f", cmap="coolwarm", center=0, square=True)
    plt.title("特征相关性热力图", fontsize=14, fontweight="bold")
    plt.tight_layout()
    filepath = os.path.join(PCA_OUTPUTS, "03_correlation_heatmap.png")
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    print(f"保存图像: {filepath}")
