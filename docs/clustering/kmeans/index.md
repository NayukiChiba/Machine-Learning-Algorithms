...
| 组件 | 路径 | 说明 |
|---|---|---|
| 数据生成 | `data_generation/clustering.py` | `ClusteringData.kmeans()` 生成二维 blob 聚类数据 |
...
| 数据来源 | `ClusteringData.kmeans()` 调用 `make_blobs(...)` |
| 特征预处理 | `StandardScaler().fit_transform(X)` |
| 训练输入 | 去掉 `true_label` 后的二维特征 |
| 评估呈现 | 聚类散点图 + `true_label` 对照 + `inertia_` 日志 |
...
- 阅读时建议始终把文档内容与 `pipelines/clustering/kmeans.py` 和 `model_training/clustering/kmeans.py` 对照起来看。
