...
> 对应代码：`pipelines/classification/decision_tree.py`、`model_training/classification/decision_tree.py`、`data_generation/classification.py`
>
...
| 组件 | 路径 | 说明 |
|---|---|---|
| 数据生成 | `data_generation/classification.py` | 生成 `decision_tree_classification_data` |
| 数据导出 | `data_generation/__init__.py` | 向外暴露 `decision_tree_classification_data` |
| 训练封装 | `model_training/classification/decision_tree.py` | 构建并训练 `DecisionTreeClassifier` |
| 流水线入口 | `pipelines/classification/decision_tree.py` | 组织切分、训练、预测与评估 |
...
- 如果只读一个文件，建议先读 `pipelines/classification/decision_tree.py`。
...
`model_training/classification/decision_tree.py` 里的 `train_model(...)` 主要负责：

1. 创建 `DecisionTreeClassifier(...)`
...
- 计算多分类 One-vs-Rest ROC 曲线
- 保存图片文件

### 特征重要性模块

`plot_feature_importance(...)` 负责：

- 接收树模型与特征名
- 读取 `feature_importances_`
- 绘制特征重要性图
...
- 当前 Decision Tree 文档必须明确：五类可视化都不是训练的一部分，而是训练完成后的诊断步骤。
- 其中特征重要性图是当前分册区别于很多其他分类分册的重要评估入口。
- 决策边界图依赖单独训练的 `model_2d`，而 ROC 曲线依赖 `predict_proba(...)` 输出的概率矩阵。
...
|---|---|
| 终端标题 | `决策树分类流水线` |
| 训练日志 | 训练耗时、最大深度、叶子节点数、`criterion` |
| 图像文件 | 混淆矩阵图、ROC 曲线图、特征重要性图、决策边界图、学习曲线图 |
...
1. 先看 `pipelines/classification/decision_tree.py`
2. 再看 `model_training/classification/decision_tree.py`
3. 再看 `result_visualization/confusion_matrix.py`
4. 再看 `result_visualization/roc_curve.py`
5. 再看 `result_visualization/feature_importance.py`
6. 再看 `result_visualization/decision_boundary.py`
7. 再看 `result_visualization/learning_curve.py`
8. 最后回到 `data_generation/classification.py`
...
3. 不区分类别预测输出、概率输出和特征重要性的用途。
4. 只看单个文件，不顺着调用链理解整体执行流程。

## 小结

- 当前 Decision Tree 工程实现采用了清晰的模块分层：数据生成、训练封装、流水线编排、结果可视化。
