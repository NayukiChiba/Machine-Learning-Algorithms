...
| `train_model(X_train_s, y_train)` | 函数 | 训练主逻辑回归模型 |
| `model.predict(X_test_s)` | 方法 | 生成测试集类别预测结果 |
| `model.predict_proba(X_test_s)` | 方法 | 生成测试集正类概率输出 |
...
- 当前逻辑回归实现使用优化器求解，标准化有助于收敛稳定，也有助于后续系数解释和 PCA 可视化。
...
- `predict_proba(...)` 是当前逻辑回归分册的重要接口，因为它直接提供类别概率输出。
- 在当前二分类实现里，ROC 曲线实际使用的是正类概率列。
- 这也是当前分册与只看 `predict(...)` 的基础分类示例不同的地方。
...
- 因此当前实现采用 `PCA` 把特征投影到二维空间，再单独训练一个二维逻辑回归模型用来画图。
- 这是整个逻辑回归分册里最需要讲清的工程细节之一。
...
- 学习曲线使用的是一个新的 `LogisticRegression(...)` 实例，而不是直接复用 `model`。
...
- 当前 Logistic Regression 流水线的训练过程非常清晰：复制数据、切分、标准化、训练主模型、测试集类别预测、概率预测、再做多种可视化诊断。
