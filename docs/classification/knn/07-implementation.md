...
> 对应代码：`pipelines/classification/knn.py`、`model_training/classification/knn.py`、`data_generation/classification.py`
>
...
| 组件 | 路径 | 说明 |
|---|---|---|
| 数据生成 | `data_generation/classification.py` | 生成 `knn_data` |
| 数据导出 | `data_generation/__init__.py` | 向外暴露 `knn_data` |
| 训练封装 | `model_training/classification/knn.py` | 构建并训练 `KNeighborsClassifier` |
| 流水线入口 | `pipelines/classification/knn.py` | 组织切分、标准化、训练、预测与评估 |
...
- 对大多数读者来说，这个命令是理解当前 KNN 工程实现的最佳入口。
- 它会依次完成数据读取、特征准备、模型训练、测试集预测、条件式概率输出和结果绘图。
- 如果只读一个文件，建议先读 `pipelines/classification/knn.py`。
...
- 文档要帮助读者看到：真正训练模型的是 `train_model(...)`，真正做测试集预测的是 `model.predict(...)`，真正做条件式概率输出的是 `predict_proba(...)`，真正画图的是各个可视化函数。

## 3. 训练模块负责什么

`model_training/classification/knn.py` 里的 `train_model(...)` 主要负责：

1. 创建 `KNeighborsClassifier(...)`
...
- 当前 KNN 文档必须明确：四类可视化都不是训练的一部分，而是训练完成后的诊断步骤。
...
1. 先看 `pipelines/classification/knn.py`
2. 再看 `model_training/classification/knn.py`
...
3. 不区分类别预测输出和条件概率输出的用途。
4. 只看单个文件，不顺着调用链理解整体执行流程。

## 小结

- 当前 KNN 工程实现采用了清晰的模块分层：数据生成、训练封装、流水线编排、结果可视化。
