...
> 对应代码：`pipelines/classification/knn.py`、`result_visualization/confusion_matrix.py`、`result_visualization/roc_curve.py`、`result_visualization/decision_boundary.py`、`result_visualization/learning_curve.py`
>
...
- 当前代码在调用前显式检查了 `predict_proba(...)` 是否存在，这体现了流水线对不同分类器接口差异的兼容考虑。
- 这也是为什么当前分册里必须强调 `predict_proba(...)` 的作用和条件边界。
...
| 参数名 | 当前对象 | 说明 |
|---|---|---|
| `model_2d` | 单独训练的二维 KNN 模型 | 用于可视化边界 |
| `X_2d` | `PCA` 降到二维后的特征 | 用于画图 |
| `y.values` | 全量标签数组 | 用于点着色 |

### 理解重点

- 这张图最适合回答：当前局部邻域分类模型在二维投影视角下，形成了怎样的分界区域。
- 它能帮助读者直观理解 KNN 的边界通常比线性模型更弯曲，也更贴近局部数据分布。
- 但它只是 PCA 投影空间中的近似展示，不是原始高维邻域关系的完整真相。
...
| 参数名 | 当前对象 | 说明 |
|---|---|---|
| `model` | `KNeighborsClassifier(n_neighbors=5)` | 用于曲线诊断的新模型实例 |
...
2. 忽略当前流水线对 `predict_proba(...)` 做了条件检查。
3. 把 PCA 决策边界图误认为原始特征空间邻域结构的完整表达。
4. 把当前仓库未实现的 accuracy、precision、recall、f1 写成现有流程的一部分。

## 小结

- 当前仓库对 KNN 的评估方式很明确：结果矩阵上看混淆矩阵，概率区分能力上看 ROC 曲线，边界形状上看 PCA 决策边界图，训练行为上看学习曲线。
- 四者组合起来，比单一指标更能解释当前 KNN 模型的实际表现。
- 对当前双月牙教学数据而言，这样的评估设计兼顾了直观性与工程可读性。
