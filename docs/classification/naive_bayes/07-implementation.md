...
> 对应代码：`pipelines/classification/naive_bayes.py`、`model_training/classification/naive_bayes.py`、`data_generation/classification.py`
>
...
| 组件 | 路径 | 说明 |
|---|---|---|
| 数据加载 | `data_generation/classification.py` | 生成 `naive_bayes_data` |
| 数据导出 | `data_generation/__init__.py` | 向外暴露 `naive_bayes_data` |
| 训练封装 | `model_training/classification/naive_bayes.py` | 构建并训练 `GaussianNB` |
| 流水线入口 | `pipelines/classification/naive_bayes.py` | 组织切分、标准化、训练、预测与评估 |
...
- 对大多数读者来说，这个命令是理解当前 Naive Bayes 工程实现的最佳入口。
- 它会依次完成数据读取、特征准备、模型训练、测试集预测、概率输出和结果绘图。
- 如果只读一个文件，建议先读 `pipelines/classification/naive_bayes.py`。
...
`model_training/classification/naive_bayes.py` 里的 `train_model(...)` 主要负责：
...
- 在二分类或多分类 One-vs-Rest 视角下计算 ROC 曲线
- 保存图片文件
...
- 当前 Naive Bayes 文档必须明确：四类可视化都不是训练的一部分，而是训练完成后的诊断步骤。
...
|---|---|
| 终端标题 | `朴素贝叶斯分类流水线` |
| 训练日志 | 训练耗时、`var_smoothing`、类别、类别先验 |
...
1. 先看 `pipelines/classification/naive_bayes.py`
2. 再看 `model_training/classification/naive_bayes.py`
...
- 当前 Naive Bayes 工程实现采用了清晰的模块分层：数据加载、训练封装、流水线编排、结果可视化。
